...
def evaluate(switcher: AcceleratorSwitcherPnPADMM, physics, args, device: torch.device):
    ...
    for idx_img, x_eval in enumerate(set5_loader):
        ...
        # physics.noise_model.sigma = torch.tensor(0.01, device=device)
        y = physics(x_eval)
        ...
        x_hat_pgd, x_hist_pgd = switcher(
            ...
            physics,
            mode="fixed",
            fixed_params={"alpha": switcher.default_rho_admm, "lambd": switcher.default_lambda, "rho": switcher.default_rho},
        ...
        if store_history:
            display_names = ["ADMM" if name == "PGD" else name for name in switcher.acc_names]
            ...
            axes[0].plot(range(len(psnr_pgd)), psnr_pgd, label="PSNR (ADMM)", linestyle="--")
            ...
            axes[2].plot(steps, alphas, label="rho_admm (penalty)")
            ...
            fig.savefig(f"figures_resultsAcc/rl_selection_spc_image{idx_img+1}admm.png", bbox_inches="tight")
            plt.close(fig)

    print("PSNR per Set5 image (deblurring):")
    for idx_img, res in enumerate(results):
        print(f"Image {idx_img+1}: ADMM={res['pgd']:.2f} | RL={res['rl']:.2f}")
    ...
    if first_curve is not None:
        ...
        plt.legend(["ADMM baseline", "RL algo selector"])
        plt.xlabel("Iteration")
        plt.ylabel("PSNR")
        plt.title("PSNR vs iteration (first Set5 image, deblurring)")
        plt.grid()
        os.makedirs("figures_resultsAcc", exist_ok=True)
        plt.savefig(f"figures_resultsAcc/rl_acc_selector_spc_cr{args.cr}_sigma{args.sigma}admm.png")
        plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rl_checkpoint", type=str, default="results/rl_acc_selector_spcADMM.pth", help="Controller checkpoint path.")
