...
def save_comparison_grid(all_images, output_dir):
    ...
    row_titles = ["Ground Truth", "Measurement", "PGD", "RL Agent"]
    ...
    for j, data in enumerate(all_images):
        ...
        # 🔹 Métricas PGD
        # =========================
        txt_pgd = f"{data['pgd_psnr']:.1f} / {data['pgd_ssim']:.3f}"
        axes[2, j].text(
            0.98, 0.02, txt_pgd,
            color="white",
            fontsize=14,
...
def evaluate_set5(switcher: AcceleratorSwitcherPnP, physics, loader: DataLoader, device: torch.device, output_dir: str):
    ...
    for idx_img, x_eval in enumerate(loader):
        ...
        for acc_name in switcher.acc_names:
            ...
            if acc_name.lower() == 'pgd':
                temp_visual['pgd'] = x_hat.cpu()
                temp_visual['pgd_psnr'] = p_curve[-1]
                temp_visual['pgd_ssim'] = s_curve[-1]
        ...
        def save_single_plot(metric_key, y_label, title_prefix, filename):
            ...
            for name, data in curves.items():
                s = styles.get(name.upper() if name != "RL Agent" else "RL Agent", {'marker': 'v', 'alpha': 0.7})
                plt.plot(data[metric_key], label=name.upper(), color=s.get('color'), marker=s.get('marker'), 
    ...
    for name, metrics in final_report.items():
        ...
        print(f"{name:<18} | {avg_p:>12.2f} dB | {avg_s:>12.4f} {tag}")
    print("="*60 + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rl_checkpoint", type=str, default="/home/hdspdeep/Usuarios/carlosmo/Agentic/AgentReg/results/rl_acc_selector_spc.pth", help="Path to controller checkpoint.")
    ...
    parser.add_argument("--rl_discount", type=float, default=0.72, help="Discount factor.")
    ...
    parser.add_argument("--output_dir", type=str, default="/home/hdspdeep/Usuarios/carlosmo/Agentic/AgentReg/figures_resultsAcc/set5_eval_SPC_sigma_0.02_cr0.7insvg", help="Directory to save plots and logs.")
